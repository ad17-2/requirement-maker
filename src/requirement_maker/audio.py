...
def extract_audio(video_path: Path, output_dir: str | None = None) -> Path:
    ensure_ffmpeg()
...
async def _split_one_chunk(
    ...
    if proc.returncode != 0:
        raise RuntimeError(f"ffmpeg failed for chunk {output_path}: {stderr.decode()}")
    return output_path


async def split_audio(file_path: Path, chunk_duration: int = 600) -> list[Path]:
    ensure_ffmpeg()
    total_duration = get_audio_duration(file_path)
    if total_duration <= 0:
        raise SystemExit(f"Could not determine duration of {file_path}")
...
async def prepare_audio(file_path: Path) -> list[Path]:
    ...
    if os.path.getsize(audio_path) > MAX_WHISPER_SIZE:
        return await split_audio(audio_path)
